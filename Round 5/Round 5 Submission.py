...
class Trader:
    ...
    POSITION_LIMIT = {'STARFRUIT' : 20, 'AMETHYSTS' : 20, 'ORCHIDS' : 100, 'GIFT_BASKET' : 60, 'CHOCOLATE' : 250, 
                      'STRAWBERRIES' : 350, 'ROSES' : 60, 'COCONUT': 300, 'COCONUT_COUPON': 600}
    ...
    def STARFRUIT_bs(
        ...
        if (best_ask < next_mid or ask_spike) and price_spread <= 3:
            return 'buy'
        elif (best_bid > next_mid or bid_spike) and price_spread <= 3:
            return 'sell'
        else: 
            return 'MM'   
    ...
    def GIFT_BASKET_bs(
        ...
        if trade_product == 'GIFT_BASKET':
            z_score = 2
            close_at = 1
            if len(mid_prices[trade_product]) >= window:
                ...
                component_basket = 4 * product_mids['CHOCOLATE'] + 6 * product_mids['STRAWBERRIES'] + product_mids['ROSES']
                mu, sigma = 343.44565, 76.21974689054737
                x = product_mids['GIFT_BASKET'] - component_basket
                z = (x - mu) / sigma
        ...
        ##############################
            ...
            rsi_window = 20 if trade_product == 'CHOCOLATE' else 40
            if trade_product in ['STRAWBERRIES', 'CHOCOLATE'] and len(mid_prices[trade_product]) >= rsi_window:
                price_diff = np.diff(mid_prices[trade_product][-rsi_window:])
    ...
    def COCONUT_bs(
        ...
        expiry = 250 / 365
        timestamp = timestamp + 3 * 1e6
        s, k, r, t = product_mids['COCONUT'], 10000, 0.0, expiry - timestamp / (365 * 1e6)
        sigma = iv_cache[trade_product].mean() if len(iv_cache[trade_product]) > 0 else 0.19527607231473124
        curr_sigma = solve_iv(product_mids['COCONUT_COUPON'], s, k, r, t) 
        
        delta = call_delta(s, k, r, curr_sigma, t)
        hedge_pos = math.ceil((pos['COCONUT_COUPON'] * delta))
        if hedge_pos > 0:
            hedge_pos = 300 - pos['COCONUT']
        elif hedge_pos < 0:
            hedge_pos = -300 - pos['COCONUT']

        theo_price = black_scholes_call(s, k, r, sigma, t)
        diff = (theo_price / product_mids['COCONUT_COUPON'])- 1
        trade_if = 0.001
        if diff > trade_if:
            cond = 'buy'
        elif diff < -trade_if:
    ...
    def run(self, state: TradingState) -> tuple[dict[Symbol, list[Order]], int, str]:
        ...
        for product in state.order_depths:    
            ...
            trades = state.market_trades[product] if product in list(state.market_trades.keys()) else None
            buyer, seller = {}, {}
            ...
            if product == 'STARFRUIT':
                ...
                if 'Rhianna' in buyer.keys() and buyer['Rhianna'] >= 5:
                        cond = 'buy'
                        buy_price, sell_price = best_ask, best_bid + 1
                if 'Rhianna' in seller.keys() and seller['Rhianna'] >= 5:
                        cond = 'sell'
                        buy_price, sell_price = best_bid, best_ask - 1
            ...
            if product == 'ORCHIDS':
                ...
                cond, conversions, mm_bid_adj, mm_ask_adj = self.ORCHIDS_bs(curr_pos, best_bid, best_ask, conversion_bid, conversion_ask, conversion_transport, 
                                                                            conversion_export, conversion_import, self.sunlight_cache, self.humidity_cache)
            ...
            if product == 'GIFT_BASKET':
                cond, bid, ask = self.GIFT_BASKET_bs(curr_pos, state.order_depths, self.mid_cache, product, self.window)
                if cond == 'buy':
            ...
            if product == 'ROSES':
                cond = 'do nth'
                if 'Rhianna' in buyer.keys():
                    cond = 'buy'
                    buy_price, sell_price = best_ask, best_bid
                if 'Rhianna' in seller.keys():
                    cond = 'sell'
                    buy_price, sell_price = best_bid, best_ask
            ...
            if product == 'COCONUT':
                ...
                if cond == 'sell':
                    buy_price, sell_price = bid, best_ask
                if 'Vinnie' in seller.keys() and seller['Vinnie'] == 4:
                    cond = 'sell'
                    buy_price, sell_price = bid, best_ask
            ...
            if cond == 'buy':
                ...
                if product in ['ROSES', 'GIFT_BASKET', 'STRAWBERRIES', 'CHOCOLATE']:
                    trade_vol = pos_limit - curr_pos
                    if trade_vol > 0:
                        orders.append(Order(product, buy_price, trade_vol))
                elif product == 'COCONUT':
                    orders.append(Order(product, buy_price, hedge_pos))
                else:
            ...
            if len(self.spread_cache[product]) > self.window:
                for i in caches:
                    i[product] = i[product][-self.window:] 
            if len(self.iv_cache[product]) > 252:
                self.iv_cache[product] = self.iv_cache[product][-252:]
